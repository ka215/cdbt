		...
		case 'download': 
			...
			if (!empty($binary_files)) {
				...
				foreach ($binary_files as $binary_file) {
					$response .= sprintf('<li><a href="%s?id=%s&filename=%s&token=%s">%s</a></li>', plugins_url(PLUGIN_SLUG) . '/lib/media.php', $binary_file['ID'], $binary_file['origin_file'], wp_create_nonce(PLUGIN_SLUG . '_download'), rawurldecode($binary_file['origin_file']));
				}
				$response .= '</ul>';
				echo $response;
			} else {
